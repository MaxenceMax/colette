...
void main() {
  ...
  test('liste vide : zéro biberon, zéro ml', () {
    expect(compute(events: const [], now: now), const RollingIntake(bottles: 0, ml: 0));
  });
  ...
  test('somme les biberons des dernières 24 h', () {
    ...
    expect(compute(events: events, now: now), const RollingIntake(bottles: 2, ml: 150));
  });
  ...
  test('un biberon à exactement 24 h est retenu', () {
    ...
    expect(compute(events: events, now: now), const RollingIntake(bottles: 1, ml: 70));
  });
  ...
  test('un biberon à plus de 24 h est exclu', () {
    ...
    expect(compute(events: events, now: now), const RollingIntake(bottles: 0, ml: 0));
  });
  ...
  test('les événements sans biberon sont ignorés', () {
    ...
    expect(compute(events: events, now: now), const RollingIntake(bottles: 1, ml: 90));
  });
}
