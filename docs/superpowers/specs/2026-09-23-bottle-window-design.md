...
- Demi-largeur : `round(I × 0,15)` minutes (3 h → 27 min, 4 h → 36 min, 2 h → 18 min).
- Bornes : `windowStart = arrondi5(nextBottleAt − demi-largeur)`, `windowEnd = arrondi5(nextBottleAt + demi-largeur)`, arrondies aux 5 minutes les plus proches (secondes ignorées).
- Aucun biberon enregistré : `windowStart = windowEnd = nextBottleAt = now`.

Exemple : 8 prises, dernier biberon à 5h10 → centre 8h10 → 7h43 → **7h45**, 8h37 → **8h35**.
...
| `windowStart ≤ now ≤ windowEnd`, bornes distinctes | « maintenant, jusqu'à 8h35 » |
...
- Biberons suivants : `ancre = max(nextBottleAt, now)`, puis centres `ancre + k × I` (k = 1, 2, …) tant que le centre est `< now + 24 h`. Chaque fourchette suit la règle ci-dessus. Un retard décale donc toute la suite.
...
- `dashboard/domain`
  - `FeedingPlan` : champs `windowStart`, `windowEnd` ; `lateBy(now)` se base sur `windowEnd`.
  - `ComputeFeedingPlan` : calcule la fourchette ; helpers statiques `intervalFor(feedsPerDay)`, `windowAround(center, interval)` (record `(DateTime, DateTime)`), `dailyTargetFor(...)` pour factoriser la cible OMS.
