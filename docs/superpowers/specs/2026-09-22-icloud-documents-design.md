...
| Opérations concurrentes | Un deuxième aperçu, sélecteur de dossier, sélecteur de fichier ou scan déclenché pendant qu'un premier est encore ouvert est refusé immédiatement avec `cancelled` ; l'UI l'ignore comme n'importe quelle annulation, sans perturber l'opération en cours. |
...
- `AsyncData`, y compris un rafraîchissement en cours (`hasValue` reste vrai pendant un `AsyncLoading` avec `previousData`) : `ListView.builder` de `DocumentEntryTile`, dans un `RefreshIndicator` qui rafraîchit le provider. La liste actuelle reste affichée pendant le tirer-pour-rafraîchir et après une écriture (scan/import réussis invalident le provider), plutôt que de basculer sur le spinner ; un échec du rafraîchissement est journalisé (`log(..., name: 'colette')`), pas relancé, pour ne pas remplacer la liste déjà affichée par une erreur.
...
7. **Opérations concurrentes explicitement refusées côté Swift.** Un deuxième aperçu, sélecteur ou scan déclenché pendant qu'un premier est en vol échoue avec `cancelled` (ignoré par l'UI) au lieu de se comporter de façon indéfinie ; ce n'était pas détaillé dans la version initiale de la spec.
