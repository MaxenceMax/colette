...
- le voir dans la page Santé, mêlé aux étapes du calendrier, et sur la carte d'accueil s'il est le prochain ;
- le marquer comme fait et noter les vaccins reçus ce jour-là, connus ou non du calendrier, avec nom commercial et lot.

Décisions prises le 2026-09-23 avec Maxence : RDV libres mêlés aux étapes (pas de section à part) ; rappel par le Calendrier iOS et la carte d'accueil, pas par le digest du matin ; vaccins de la liste connue plus des vaccins à nom libre.
...
- `HealthCard` : si `timeline.next` est un `AppointmentItem`, affiche son titre et son texte de statut ; la règle « étape lointaine en une ligne » ne s'applique qu'aux étapes.
...
- **Présentation** (`pumpApp`, `mocktail`) : page (bouton « + », RDV libre mêlé à la bonne section, tuile avec pastilles) ; feuille (création avec titre vide refusée, création valide appelle `save` avec l'`id` du `FixedIdGenerator`, édition, ajout d'un vaccin connu et d'un vaccin libre, suppression confirmée, bouton inactif pendant l'écriture) ; contrôleur (validation, écriture, suppression, sync appelée) ; carte d'accueil avec un RDV libre en prochain ; `FirestoreHealthSync` réconcilie aussi les RDV libres et n'écrit rien si leur lecture serveur échoue ; `HealthSyncGate` relance sur émission des RDV libres.
...
- Conflits prévisibles : `app_fr.arb`, `failure.dart`, `failure_message.dart`, `health_page.dart`, `health_sync.dart`.
